...
var server = http.createServer(async function (req, res) {
    ...
    if (req.method === 'POST') {
		...
		if(url.pathname === "/api/causeerror") {
			...
			throw "Dynatrace Test Exception";    // throw a text
	...
	else if (req.url.startsWith("/BD_Cartoon_Shout-webfont.ttf")) {

		var ttf = fs.readFileSync("BD_Cartoon_Shout-webfont.ttf");
		res.writeHead(200, 'OK', {'Content-Type': 'application/x-font-ttf'});
	        res.write(ttf);
		res.end();

		}
	else if (req.url.startsWith("/pacman.js")) {

		var pac = fs.readFileSync("pacman.js");
		res.writeHead(200, 'OK', {'Content-Type': 'text/javascript'});
	        res.write(pac);
		res.end();

		}
	else if (req.url.startsWith("/modernizr-1.5.min.js")) {
	var modern = fs.readFileSync("modernizr-1.5.min.js");
	res.writeHead(200, 'OK', {'Content-Type': 'text/javascript'});
	res.write(modern);
	res.end();
